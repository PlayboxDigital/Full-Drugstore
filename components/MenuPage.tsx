...
const menuData: MenuCategory[] = [
  {
    ...
    items: [
      ...
      { name: "Café Chico + Frutos Secos", price: "$4.500", description: "Energía saludable y café recién hecho." },
          ...
          {menuData.map((category, index) => {
            ...
            return (
              ...
              >
                ...
                {isPromo && (
                  <>
                    <div className="absolute top-0 right-0 bg-brand-yellow text-brand-dark px-6 py-1.5 rounded-bl-2xl font-display text-xs uppercase tracking-widest flex items-center gap-1 shadow-sm">
                    ...
                    <div key={idx} className="group flex items-start gap-5">
                      ...
                      <div className="flex-grow">
                        <div className={`flex items-baseline justify-between border-b ${isPromo ? 'border-brand-yellow/20' : 'border-gray-200'} pb-2 mb-2 border-dashed`}>
                           <span className="font-serif text-lg text-gray-800">{item.name}</span>
                           <span className={`font-sans font-bold text-lg ${category.textColor}`}>{item.price}</span>
                        </div>
                        {item.description && (
